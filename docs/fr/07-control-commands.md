...
- **Design decisions**: D12 (control commands `status` and `cancel` bypass the workflow queue and are handled immediately; `cancel` kills the active run's child process tree and posts cancellation in-thread). The best-effort process-tree kill and its clean-cancellation follow-up are documented in `README.md` (Known v1 limitations). The `done`/`close` and `stop`/`abort` aliases extend D12's command set for the resident lifecycle.
