...
- **Design**: see `docs/superpowers/specs/2026-06-14-phase3-resident-agent-design.md` — the Phase 3 design of record. Components §1 (`ResidentSessions` registry), §2 (promotion at launch), §3 (ingress short-circuit), §4 (bridge routes by either registry, splits the `result` handler by turn), §5 (cmux host handle in `index.ts`), plus Lifecycle & control (`done` / `stop`, no wall-clock reaping) and Error handling (dead-surface recovery, launch-failure cleanup). Routing, addressing, hierarchy, idle reaping, and per-run `stop` are marked deferred there. The live behaviour is exercised in `docs/smoke-phase3-resident.md`.
